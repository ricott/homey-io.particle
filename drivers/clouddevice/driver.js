...
class CloudDeviceDriver extends Homey.Driver {
    ...
    _registerFlows() {
        ...
        //Conditions
        const particle_variable_condition = this.homey.flow.getConditionCard('particle_variable_condition');
        particle_variable_condition.registerRunListener(async (args, state) => {
        ...
        particle_variable_condition.registerArgumentAutocompleteListener('variable',
            async (query, args) => {
                return args.device.device.variables;
            }
        );
        particle_variable_condition.registerArgumentAutocompleteListener('conditionType',
            async (query, args) => {
                return variableConditionList;
            }
        );

        const particle_function = this.homey.flow.getActionCard('particle_function');
        particle_function.registerRunListener(async (args) => {
        ...
        particle_function.registerArgumentAutocompleteListener('function',
            async (query, args) => {
                return args.device.device.functions;
            }
        );

        const particle_event = this.homey.flow.getActionCard('particle_event');
        particle_event.registerRunListener(async (args) => {
            this.log(`[${args.device.getName()}] Action triggered`);
    ...
    publishEvent(eventName, data, isPrivate) {
        ...
        return new Particle().publishEvent({ name: eventName, data: data, isPrivate: isPrivate, auth: this.homey.settings.get('access_token') })
            .then(function (data) {
                ...
                self.log("Failed to publish event", reason);
                return false;
            });
    }
    ...
    async onPairListDevices() {
        ...
        return particle.listDevices({ auth: self.homey.settings.get('access_token') })
            .then(function (response) {
                ...
                self.log('List devices call failed: ', reason);
                throw reason;
            });
    }
